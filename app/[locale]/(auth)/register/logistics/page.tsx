...
export default function RegisterLogisticsPage({ params: { locale } }: { params: { locale: string } }) {
  ...
  const onSubmit = async (data: RegisterInput) => {
    ...
    try {
      const res = await fetch('/api/register', {
        ...
        body: JSON.stringify({ ...data, companyType: 'LOGISTICS' }),
      })
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 flex items-center justify-center p-4">
      <div className="w-full max-w-lg">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 text-sm">{t('logisticsDesc')}</p>
        </div>

        <Card className="border-0 shadow-2xl">
          <CardHeader className="pb-4">
            <CardTitle>{t('registerLogistics')}</CardTitle>
            <CardDescription>Create your logistics company account</CardDescription>
          </CardHeader>
          <CardContent>
            {error && (
              ...
              <div className="border-t pt-4">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <Input {...register('companyName')} className="mt-1" placeholder="Swift Cargo LLC" />
